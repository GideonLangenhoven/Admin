...
export default function GuidePhotosPage({ params }: { params: Promise<{ slotId: string }> }) {
  ...
  var [photos, setPhotos] = useState<Photo[]>([]);
  var [uploading, setUploading] = useState(false);
  var [progress, setProgress] = useState<{ done: number; total: number } | null>(null);
  var [emailStatus, setEmailStatus] = useState<string | null>(null);
  ...
  async function reload() {
    ...
    var [photosRes, slotRes] = await Promise.all([
  ...
  async function onPickPhotos(files: FileList | null) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      try {
        var r = await fetch("/api/guide/photo-upload", { method: "POST", body: fd });
  ...
  async function sendThankYou() {
    ...
    try {
      var r = await fetch("/api/guide/send-thank-you", {
        method: "POST",
      ...
      {photos.length > 0 && (
        <>
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            {photos.map(p => {
              var url = p.gdrive_view_url || p.photo_url;
              return (
