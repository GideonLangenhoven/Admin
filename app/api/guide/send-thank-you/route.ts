...
export async function POST(req: NextRequest) {
  ...
  var tourName = (slot as any).tours?.name || "your adventure";
  ...
  var { data: photos } = await db.from("trip_photos")
    .select("photo_url, gdrive_view_url")
    .eq("slot_id", slot_id)
    .eq("business_id", caller.business_id);

  var photoUrls = (photos || []).map((p: any) => p.gdrive_view_url || p.photo_url).filter(Boolean);
  var photoUrl = photoUrls[0] || "";

  var { data: biz } = await db.from("businesses")
  ...
  var results: { booking_id: string; ok: boolean; error?: string }[] = [];
  for (var b of bookings) {
    if (!b.email) continue;
    try {
      var r = await fetch(supabaseUrl + "/functions/v1/send-email", {
        method: "POST",
