...
begin
  select coalesce(max(jobid), 0) + 1 into next_id from cron.job;
  insert into cron.job(jobid,jobname,schedule,command) values(next_id,job_name,job_schedule,job_command);
  return next_id;
end $$;
