...
function fixture({ free = false, soldOut = false, retry = false, email = "guest@example.invalid" } = {}) {
  ...
  const checkout = vi.fn(async () => ({ data: soldOut ? { reason: "No spots available" } : { redirectUrl: "https://checkout.example.invalid", amount: free ? 0 : 100, expires_at: new Date(Date.now()+900000).toISOString() } }));
  ...
  return {submit,writes,checkout,showToast,setStep,navigate,setDraftBookingId};
}
...
describe("storefront authoritative checkout", () => {
  for(const free of [false,true]) it(`sends proof, promo, vouchers and extras for ${free?'free':'cash'} checkout`,async()=>{
    ...
    expect(f.navigate).toHaveBeenCalledWith("https://checkout.example.invalid");
