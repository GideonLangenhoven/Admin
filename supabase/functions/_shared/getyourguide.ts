...
const PROD_BASE = "https://supplier-api.getyourguide.com";
const SANDBOX_BASE = "https://supplier-api.sandbox.getyourguide.com";
...
export function createGygClient(opts: {
  ...
  const fetchFn = async (path: string, init: RequestInit = {}): Promise<Response> => {
    const token = await getAccessToken(baseUrl, opts.clientId, opts.clientSecret);
...
export async function gygPushAvailability(client: GygClient, payload: any): Promise<any> {
  const r = await client.fetch("/supplier/availability", {
    method: "PUT",
...
export async function gygFetchBookings(client: GygClient, modifiedSince: string): Promise<any[]> {
  const r = await client.fetch("/supplier/bookings?modified_since=" + encodeURIComponent(modifiedSince));
  if (!r.ok) {
    const body = await r.text().catch(() => "");
