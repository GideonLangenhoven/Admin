...
async function sendText(to: any, t: any, bookingId?: any) {
  ...
  await recordWaMessage(BUSINESS_ID, { to: to, kind: "text", body: t, status: res.ok ? "SENT" : "FAILED", providerMessageId: data?.messages?.[0]?.id || null, error: res.ok ? null : String(data?.error?.message || "WhatsApp send failed"), bookingId: bookingId || null });
}
