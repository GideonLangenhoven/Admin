-- POPIA data-subject obfuscation — full, working implementation (item 18).
--
-- The prior anonymize_customer() had never actually run: (a) the fulfill route
-- hard-rejects any request with a null customer_id, and every customer-submitted
-- request lands with customer_id=NULL (the submit form never resolves it); and
-- (b) even if reached, it referenced bookings.notes, bookings.updated_at,
-- marketing_contacts.email_lower, and pgcrypto's digest() in a search_path that
-- excludes the extensions schema — all of which would have thrown. It also read
-- the customer's email for the marketing delete AFTER overwriting it, and
-- covered only 3 tables, ignoring invoices, vouchers, conversations,
-- chat_messages, wa_messages, and the PII inside bookings JSONB.
--
-- This version (verified end-to-end against the live schema):
--  * captures the subject's original email + phone BEFORE anonymizing,
--  * matches phones on their last 9 significant digits so 0xx / 27xx / +27
--    formats of the same SA number all match,
--  * matches by customer_id OR original email OR phone to sweep orphan rows,
--  * scrubs every PII field across all relevant tables,
--  * PRESERVES financial/legal record shells (amounts, invoice numbers,
--    voucher codes/values/balances, booking refs/qty/dates) for SARS 5-year
--    retention — only human-identifying fields are removed,
--  * accepts an optional p_email so orphan-only subjects (no customers row)
--    are still fully anonymized.
--
...
BEGIN
  ...
  UPDATE conversations SET phone = v_token, customer_name = 'Deleted Customer', email = NULL, state_data = '{}'::jsonb, updated_at = now()
