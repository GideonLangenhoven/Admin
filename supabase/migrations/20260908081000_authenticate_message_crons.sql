...
-- Deployment prerequisite: provision edge_jobs_service_role_key in Supabase
-- Vault with the SAME service-role key configured on the edge functions.
-- Never put the key in a migration or in cron.job.command itself.
do $$
declare
  job record;
  job_url text;
    ...
    perform cron.alter_job(job.jobid, command := format($command$
      select net.http_post(
        ...
        headers := jsonb_build_object(
          ...
          'Authorization', 'Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name = 'edge_jobs_service_role_key')
        ),
        body := %L::jsonb
      );
