...
| --- | --- |
| The [public pricing page](https://bookingtours.co.za/pricing) advertises R2,000 base, R750 per additional admin, R500 seasonal pause and no setup charge; card processing is separate. | Use R2,000 only as the planning base. Reconcile the complete quote, usage allowances, VAT treatment and invoice preview before accepting a customer. |
| The [July pricing migration](../../supabase/migrations/20260716120000_single_plan_pricing.sql) specifies R500 extra seats. The [BRD](../BRD.md) describes free pauses; the [billing API](../../app/api/billing/subscription/route.ts) retains a R1,500 fallback. | This proves conflicting sources, not what a live customer is currently charged. Agree one price schedule and verify it against production; do not copy old launch pricing. |
