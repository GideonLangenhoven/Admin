...
- Active writer lock: `COR-02A` for truthful voucher-reminder provider acceptance and safe retry behavior in `cron-tasks`/`send-email`. The first review requested three corrections; the follow-up now freezes provider payloads durably, rechecks the deletion boundary before each send, and validates provider IDs, with a fresh independent review pending. The additive migration is local-only until the target ledger is verified. Refund and pricing locks remain in force.
