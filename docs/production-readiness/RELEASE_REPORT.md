...
Admin application code is `18f601e3`: C02's reviewed authentication/recovery changes are integrated and 46 affected integration tests pass. The separate combo-null-gap correction is accepted, with nine actual validator and eleven parser tests passing and an identical integrated tree.
...
C01 remains open for supported runtime and CI alignment: local Node 22.22.1 predates current security patches and several jobs select EOL Node 20. Sol has obtained official checksum-verified Node 22.23.2, after which relevant runtime/build checks and source pins must align. C05 TLS/permission work remains under review; the unchanged application database suite passed 196/196 on a fresh disposable PostgreSQL 17 fixture. The independently reviewed V03 window guard is integrated at b29767b, with 10/10 offline integration tests on Node 22.23.2. It rejects stale windows and preserves failed-run/invariant evidence on expiry or ordinary interruption; hard host/process loss still needs external supervision. C01 runtime/CI alignment is active across all three apps; floating Edge resolutions remain a separate C01 gap. C03/C04/C06 and broader V01/V02 verification remain.
