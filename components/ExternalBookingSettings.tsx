...
import { confirmAction, notify } from "../app/lib/app-notify";
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-white/70 px-2.5 py-1.5">
        ...
        <button data-demo-action="external.copy"
          type="button"
          onClick={handleCopy}
...
export default function ExternalBookingSettings({ tours }: { tours: TourOption[] }) {
  ...
  async function handleDeleteMapping(id: string) {
    if (!await confirmAction({ title: "Delete mapping", message: "Delete this mapping?", tone: "warning", confirmLabel: "Delete" })) return;
    ...
    notify({ title: "Mapping deleted", message: "The external product mapping was removed.", tone: "success" });
  }

  async function handleDeleteCredential(id: string) {
    if (!await confirmAction({
      title: "Delete credential",
      message: "Delete this credential? This will immediately revoke that business/source API key.",
      tone: "warning",
    ...
    notify({ title: "Credential deleted", message: "The external booking credential was revoked.", tone: "success" });
  }
    ...
    <div className="mt-10 border-t border-[var(--ck-border-subtle)] pt-10 space-y-10">
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_0.95fr] gap-8">
                    ...
                    <div key={row.id} className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-col items-end gap-2 text-xs font-medium">
                          ...
                          <button data-demo-action={row.active ? "external.disable-key" : "external.enable-key"} onClick={() => handleToggleCredential(row)} className="text-amber-600 hover:underline">{row.active ? "Disable" : "Enable"}</button>
                          <button data-demo-action="external.delete-key" onClick={() => handleDeleteCredential(row.id)} className="text-[var(--ck-danger)] hover:underline">Delete</button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div>
            <form data-demo-submit="external.save-key" onSubmit={handleSaveCredential} className="ui-surface rounded-2xl border border-[var(--ck-border-subtle)] p-5 space-y-4">
              <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-8">
                    ...
                    <div key={row.id} className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-col items-end gap-2 text-xs font-medium">
                          ...
                          <button data-demo-action={row.active ? "external.disable-map" : "external.enable-map"} onClick={() => handleToggleMapping(row)} className="text-amber-600 hover:underline">{row.active ? "Disable" : "Enable"}</button>
                          <button data-demo-action="external.delete-map" onClick={() => handleDeleteMapping(row.id)} className="text-[var(--ck-danger)] hover:underline">Delete</button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div>
            <form data-demo-submit="external.save-map" onSubmit={handleSaveMapping} className="ui-surface rounded-2xl border border-[var(--ck-border-subtle)] p-5 space-y-4">
              <div>
